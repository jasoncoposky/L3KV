#include "engine/store.hpp"
#include <chrono>
#include <iostream>
#include <vector>
#include <filesystem>
#include <string>
#include <thread>
#include <atomic>

using namespace l3kv;

int main() {
    std::string path = "bench_engine_threaded.wal";
    std::filesystem::remove(path);
    
    try {
        {
            Engine db(path, 1);
            
            const int iterations_per_thread = 100000;
            const int threads = 16;
            std::string payload = "{\"age\": 25, \"name\": \"John Doe\", \"active\": true, \"score\": 98.5}";
            
            std::cout << "Benchmarking Engine (Async Path) with " << iterations_per_thread * threads << " ops on " << threads << " threads..." << std::endl;
            
            std::vector<std::thread> workers;
            auto start = std::chrono::high_resolution_clock::now();
            
            for (int t = 0; t < threads; ++t) {
                workers.emplace_back([&, t]() {
                    for (int i = 0; i < iterations_per_thread; ++i) {
                        db.put("u" + std::to_string(t) + "_" + std::to_string(i), payload);
                    }
                });
            }
            
            for (auto &w : workers) w.join();
            
            std::cout << "Waiting for shards to sync..." << std::endl;
            db.wait_all_shards();
            
            auto end = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> diff = end - start;
            
            double ops_per_sec = (iterations_per_thread * threads) / diff.count();
            std::cout << "Engine Throughput: " << (int)ops_per_sec << " ops/sec" << std::endl;
            std::cout << "Total Time: " << diff.count() << "s" << std::endl;
        }
    } catch (const std::exception& e) {
        std::cerr << "Bench Error: " << e.what() << std::endl;
        return 1;
    }
    
    std::filesystem::remove(path);
    return 0;
}
